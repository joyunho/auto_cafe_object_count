...
import * as maps from '../src/data/pos-map.js';
import { SEED_ITEMS } from '../src/data/items.js';

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const dir = path.resolve(process.argv[2] || path.join(root, 'data', 'pos'));
...
const model = {
  version: 1,
  source: `POS 그룹별 매출분석 ${sales.months[0]} ~ ${sales.months.at(-1)} × 레시피 2026.08`,
  months: sales.months,
  items: Object.fromEntries(
    rows
      .filter((r) => r.perPackage && itemIndex[r.itemId])
      .map((r) => [r.itemId, { perDay: Object.fromEntries(sales.months.map((m) => [m, Math.round((r.perDay[m] || 0) * 1000) / 1000])), avgPerDay: Math.round(r.avgPerDay * 1000) / 1000, unit: r.unit, assumed: r.assumed }]),
  ),
};
